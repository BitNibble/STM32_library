...
{ // slow
	uint8_t status = 255;
	STM32446TimeTr = rtc_stm32446.rtc.reg->TR;
	STM32446DateDr = rtc_stm32446.rtc.reg->DR;
	
	status = STM32446RtcAccess(clock);
	
	// Some help from youtube vids
	//rtc_stm32446.rcc.reg->CFGR &= (uint32_t) ~((1 << 0) | (1 << 1)); // RCC_CFGR sw[2:0] 00 Bits 1:0
	...
	rtc_stm32446.rtc.reg->WPR |= 0xCA;
	rtc_stm32446.rtc.reg->WPR |= 0x53;
	//rtc_stm32446.rtc.reg->ISR &= (uint32_t) ~((1 << 3) | (1 << 5) | (1 << 7));

	// 6 - Set INIT bit and wait for ready flag
	rtc_stm32446.rtc.reg->ISR |= (1 << 7); // INIT: Initialization mode
	while( !(rtc_stm32446.rtc.reg->ISR & (1 << 6)) ); // INITF: Initialization flag
	status = 6;

	// 7 - Adjust prescaler values for RTC to obtain 1 Hz
	//rtc_stm32446.rtc-reg->PRER |= 0xF9;
	//rtc_stm32446.rtc.reg->PRER |= (0x7F << 16);
	
	// 8 - Write Time and Date values
	//rtc_stm32446.rtc.reg->TR |= 0x130000;
	//rtc_stm32446.rtc.reg->TR |= 0x5700;
	//rtc_stm32446.rtc.reg->DR |= 0x215124;
	
	// 9 - Set BYPSHAD bit
	// must read twice
	//rtc_stm32446.rtc.reg->CR |= (1 << 5); // BYPSHAD: Bypass the shadow registers
	// read only once
	rtc_stm32446.rtc.reg->CR &= (uint32_t) ~(1 << 5); // BYPSHAD: Disable Bypass the shadow registers
	
	// 10 - Clear INIT bit
	rtc_stm32446.rtc.reg->ISR &= (uint32_t) ~(1 << 7);
	
	// 11 - Disable access to RTC registers
	rtc_stm32446.pwr.reg->CR &= (uint32_t) ~(1 << 8);
...
void STM32446RtcHour(uint8_t hour)
{
	...
	t = rtc_stm32446.func.dec2bcd(hour / 10);
	u = rtc_stm32446.func.dec2bcd(hour % 10);
	STM32446TimeTr = rtc_stm32446.rtc.reg->TR;
...
void STM32446RtcMinute(uint8_t minute)
{
	...
	t = rtc_stm32446.func.dec2bcd(minute / 10);
	u = rtc_stm32446.func.dec2bcd(minute % 10);
	STM32446TimeTr = rtc_stm32446.rtc.reg->TR;
...
void STM32446RtcSecond(uint8_t second)
{
	...
	t = rtc_stm32446.func.dec2bcd(second / 10);
	u = rtc_stm32446.func.dec2bcd(second % 10);
	STM32446TimeTr = rtc_stm32446.rtc.reg->TR;
...
void STM32446RtcYear(uint8_t year)
{
	...
	t = rtc_stm32446.func.dec2bcd(year / 10);
	u = rtc_stm32446.func.dec2bcd(year % 10);
...
void STM32446RtcWeekDay(uint8_t weekday)
{
	...
	u = rtc_stm32446.func.dec2bcd(weekday % 10);
...
void STM32446RtcMonth(uint8_t month)
{
	...
	t = rtc_stm32446.func.dec2bcd(month / 10);
	u = rtc_stm32446.func.dec2bcd(month % 10);
...
void STM32446RtcDay(uint8_t day)
{
	...
	t = rtc_stm32446.func.dec2bcd(day / 10);
	u = rtc_stm32446.func.dec2bcd(day % 10);
...
void STM32446RtcRegWrite(volatile uint32_t* reg, uint32_t data)
{
	...
	rtc_stm32446.pwr.reg->CR |= (1 << 8); // Disable backup domain write protection
...
void STM32446Rtcdr2vec(char* vect)
{
	uint32_t dr = rtc_stm32446.rtc.reg->DR;
	// YT
	vect[0] = (uint8_t) (dr >> 20) & 0x0F;
	vect[0] = rtc_stm32446.func.bcd2dec(vect[0]);
	// YU
	vect[1] = (uint8_t) (dr >> 16) & 0x0F;
	vect[1] = rtc_stm32446.func.bcd2dec(vect[1]);
	// WDU
	vect[2] = (uint8_t) (dr >> 13) & 0x07;
	vect[2] = rtc_stm32446.func.bcd2dec(vect[2]);
	// MT
	vect[3] = (uint8_t) (dr >> 12) & 0x01;
	vect[3] = rtc_stm32446.func.bcd2dec(vect[3]);
	// MU
	vect[4] = (uint8_t) (dr >> 8) & 0x0F;
	vect[4] = rtc_stm32446.func.bcd2dec(vect[4]);
	// DT
	vect[5] = (uint8_t) (dr >> 4) & 0x03;
	vect[5] = rtc_stm32446.func.bcd2dec(vect[5]);
	// DU
	vect[6] = (uint8_t) dr & 0x0F;
	vect[6] = rtc_stm32446.func.bcd2dec(vect[6]);
	// Store Value
	STM32446DateDr = dr;
}
...
void STM32446Rtctr2vec(char* vect)
{
	uint32_t tr = rtc_stm32446.rtc.reg->TR;
	if(rtc_stm32446.rtc.reg->ISR & (1 << 5)){ // RSF: Registers synchronization flag
		// ht
		...
		rtc_stm32446.rtc.reg->ISR &= (uint32_t) ~(1 << 5);
	}
}
...
void STM32446RtcSetTr(void)
{
	...
	rtc_stm32446.rtc.reg->TR = STM32446TimeTr;
...
void STM32446RtcSetDr(void)
{
	...
	rtc_stm32446.rtc.reg->DR = STM32446DateDr;
...
uint8_t STM32446RtcAccess(uint8_t clock)
{
	...
	rtc_stm32446.rcc.reg->APB1ENR |= (1 << 28); // Power interface clock enable
	...
	rtc_stm32446.rcc.lenable(clock);
	status = 1;
	//4 - RTCSEL[1:0]: RTC clock source selection
	rtc_stm32446.rcc.lselect(clock);
	status = 2;
	return status;
}
